...
export default function ProgramPage() {
  ...
  const acceptedPapers = articles.filter(a => 
    a.conferenceId === Number(id) && a.status === PAPER_STATUS.ACCEPTED
  );
          ...
          <div className="grid gap-6">
            {acceptedPapers.map((paper) => (
              <div key={paper.id} className="bg-white rounded-[40px] border border-slate-100 p-10 shadow-sm hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.06)] transition-all group">
                ...
                <p className="text-slate-500 text-sm mb-8 line-clamp-2 leading-relaxed font-medium">{paper.abstract || paper.description}</p>
